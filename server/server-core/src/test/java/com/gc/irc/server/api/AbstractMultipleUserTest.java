...
public abstract class AbstractMultipleUserTest extends AbstractServerTest {
	 ...
	 */
	protected final IRCMessage buildSimpleMessage(final IRCUser user,
	 ...
	 * @return the iRC message
	 */
	protected final IRCMessage buildSimplePrivateMessage(final IRCUser user,
			final String message, final int toId) {
	 ...
	 */
	protected final void checkMessageReceived(final IRCUser userSources,
			final String messageStr, final IRCMessageChat receivedChatMessage) {
		checkMessageReceived(userSources, messageStr, Arrays
				.asList(receivedChatMessage));
	}
	...
	protected final void checkMessageReceived(final IRCUser userSources,
			...
			final List<IRCMessageChat> receivedChatMessages) {
	 ...
	 */
	protected final IRCMessage sendMessageAndWaitForResponse(
			final ConnectionHandler connectionThreadSender,
			final IMessageHandlerTester messageHandlerUserDestination,
			final IRCMessage sendedMessage) throws InterruptedException {
		return sendMessageAndWaitForResponse(connectionThreadSender,
				Arrays.asList(messageHandlerUserDestination), sendedMessage)
				.get(0);
	}
	 ...
	 *             the interrupted exception
	 */
	protected final List<IRCMessage> sendMessageAndWaitForResponse(
			final ConnectionHandler connectionThreadSender,
			final List<IMessageHandlerTester> messageHandlerUserDestination,
			final IRCMessage sendedMessage) throws InterruptedException {

		if (messageHandlerUserDestination == null
				|| messageHandlerUserDestination.isEmpty()) {
			return null;
		}
		...
		for (final IMessageHandlerTester messageHandler : messageHandlerUserDestination) {
			...
			final IRCMessage receivedMessage = messageHandler
	 ...
	 *             the interrupted exception
	 */
	protected final void sendMessageToGlobal(final IRCUser userSrc,
			final ConnectionHandler connectionThreadSrc,
			final String messageStr,
			final List<IMessageHandlerTester> messageHandlerUserDest)
			throws InterruptedException {
		final IRCMessage currentSendedMessage = buildSimpleMessage(userSrc,
				messageStr);
		final List<IRCMessage> receivedMessages = sendMessageAndWaitForResponse(
				connectionThreadSrc, messageHandlerUserDest,
	 ...
	 *             the interrupted exception
	 */
	protected final void sendMessageToGlobal(
			final UserContextEntity contextSrc, final String messageStr,
			final List<UserContextEntity> contextsDest)
			throws InterruptedException {
		...
		sendMessageToGlobal(contextSrc.getUser(), contextSrc
				.getConnectionUser(), messageStr, messageHandlerUserDest);
	 ...
	 *             the interrupted exception
	 */
	protected final void sendPrivateMessage(final IRCUser userSource,
			final ConnectionHandler connectionThreadSource,
			final String messageStr,
			final IMessageHandlerTester messageHandlerUserDestination,
			final IRCUser userDestination,
			final List<IMessageHandlerTester> otherUsersMessageHandlers)
			throws InterruptedException {
		final IRCMessage currentSendedMessage = buildSimplePrivateMessage(
				userSource, messageStr, userDestination.getId());
		if (otherUsersMessageHandlers != null
				&& !otherUsersMessageHandlers.isEmpty()) {
	 ...
	 *             the interrupted exception
	 */
	protected final void sendPrivateMessage(final UserContextEntity contextSrc,
			final String messageStr, final UserContextEntity contextDest,
			final List<UserContextEntity> otherUsersContexts)
			throws InterruptedException {
		...
		if (otherUsersContexts != null && !otherUsersContexts.isEmpty()) {
			for (final UserContextEntity contextEntity : otherUsersContexts) {
				otherUsersMessageHandlers
						.add(contextEntity.getMessageHandler());
			}
		}
		sendPrivateMessage(contextSrc.getUser(),
				contextSrc.getConnectionUser(), messageStr, contextDest
						.getMessageHandler(), contextDest.getUser(),
				otherUsersMessageHandlers);
	}
